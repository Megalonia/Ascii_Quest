...
use specs::prelude::*;
use super::{Stats, Player, logs, Map, Name, Position, Point, State,InBackpack};
...
pub fn draw_ui(ecs: &World, ctx: &mut Rltk) {
    ...
    for(_player,stat) in (&players, &stats).join() {
        let health = format!("HP: {} / {}", stat.hp, stat.max_hp);
        ctx.print_color(1,43, RGB::named(rltk::LIGHT_GREEN), RGB::named(rltk::BLACK), &health);

        ctx.draw_bar_horizontal(12,43,39, stat.hp, stat.max_hp, RGB::named(rltk::GREEN), RGB::named(rltk::BLACK));

        let log = ecs.fetch::<logs::GameLog>();

        let mut y = 44;
        for s in log.entries.iter().rev() {
            if y < 49 { ctx.print(1,y, s); } 
            y += 1;
        }
...
fn draw_tooltip(ecs: &World, ctx : &mut Rltk) {
    ...
    for(name, position) in (&names, &positions).join() {
        let idx = map.find_xy_index(position.x, position.y);
