...
use specs::prelude::*;
use std::cmp::{max, min};
use super::{Position, Player, Viewshed, State, Map, RunState, CombatStats, WantsToMelee, Item, TileType, Monster,
...
pub fn try_next_level(ecs: &mut World) -> bool {
    ...
    if map.tiles[player_idx] == TileType::Hole {
        true 
    }else {
...
pub fn player_input(game_state: &mut State, ctx: &mut Rltk) -> RunState {
    ...
    match ctx.key {
        ...
        Some(key) => match key {
            ...
            //Diagonals
            VirtualKeyCode::Q => try_move_player(-1, -1, &mut game_state.ecs),
            VirtualKeyCode::E => try_move_player(1, -1, &mut game_state.ecs),
            VirtualKeyCode::A => try_move_player(-1, 1, &mut game_state.ecs),
            VirtualKeyCode::D => try_move_player(1, 1, &mut game_state.ecs),
            //Pick Up Item
